...
################################################################################
@click.command()
@click.argument('dataset_name', type=click.Choice(['mnist', 'cifar10', 'mrnet']))
@click.argument('net_name', type=click.Choice(['mnist_LeNet', 'cifar10_LeNet', 'cifar10_LeNet_ELU', 'mrnet_alexnet']))
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model, objective, nu, device, seed,
    ...
    deep_SVDD = DeepSVDD(dataset_name, cfg.settings['objective'], cfg.settings['nu'])
    ...
    if pretrain:
        ...
        deep_SVDD.pretrain(dataset_name, dataset,
    ...
    # Test model
    deep_SVDD.test(dataset_name, dataset, device=device, n_jobs_dataloader=n_jobs_dataloader)
    ...
    if dataset_name in ('mnist', 'cifar10', 'mrnet'):
